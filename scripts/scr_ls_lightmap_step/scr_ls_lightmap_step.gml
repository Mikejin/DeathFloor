...
if(lightmap_update_count >= _ls_lightmap_update_by_fps && _ls_use_lightmap_update) {
    ...
    with(obj_ls_light) {
        if(            
            x + radius >= __view_get( e__VW.XView, 0 ) &&
            x - radius <= __view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ) &&
            y + radius >= __view_get( e__VW.YView, 0 ) &&
            y - radius <= __view_get( e__VW.YView, 0 ) + __view_get( e__VW.HView, 0 )
        )
            scr_ls_light_update();
    }
